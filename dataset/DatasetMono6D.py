...
class MyDataset(Dataset):
    def __init__(self, root, cls, is_train=True, scene=None, index=None):
        ...
        if is_train:
            ...
            # 相机内参
            self.K = np.array([[2209.878296, 0.0, 349.751312],
...
# 获取训练数据list
    def get_train_data_path(self, root, cls):
        ...
        render_edge_dir = osp.join(render_dir, "gtEdge")
    ...
    def get_data(self, path):
        ...
        if path["type"] == "true":
            ...
            gt_contour = cv2.resize(gt_contour, dsize=(640, 480))
            # 膨胀？
    ...
    def random_crop(self, img):
        h, w = img.shape[:2]
        y = np.random.randint(0, h - 480)  # 480
        x = np.random.randint(0, w - 640)  # 640
        image = img[y:y + 480, x:x + 640, :]  # 480 # 640
        # cv2.imshow("",image)
        # cv2.waitKey(0)
        return image
