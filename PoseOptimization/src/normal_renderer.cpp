...
std::string NormalRenderer::fragment_shader_code_ =
	...
	"void main()\n"
	"{\n"
	"    vec3 normal_temp = vec3(Normal.z,Normal.y,Normal.x);\n"
	"	 FragColor = vec4((0.5 - 0.5 * normal_temp), 1.0).zyxw;\n"
    "}";
//0.5*Normal.z,0.5*Normal.y,Normal.x
...
bool NormalRenderer::FetchNormalImage() {
  ...
  /*cv::imwrite("normal_image_.png", normal_image_);
  cv::namedWindow("normal_image_",0);
  cv::imshow("normal_image_", normal_image_);
  cv::waitKey(0);*/
  return true;
}
...
bool NormalRenderer::FetchDepthImage() {
  ...
  //cout << depth_image_.at<ushort>(cv::Point(0,0)) << endl;
  //depth_image_.convertTo(depth_image_, CV_8UC1, 255.0 / 65535.0);
  //cv::namedWindow("depth", 0);
  //cv::imshow("depth", depth_image_);
  //cv::imwrite("depth.jpg", depth_image_);
  //cv::waitKey(0);
  return true;
}
