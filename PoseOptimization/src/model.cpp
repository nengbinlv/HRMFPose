...
void Model::set_directory(const std::experimental::filesystem::path &directory) {
  directory_ = directory;
  set_up_ = false;
}
...
const std::experimental::filesystem::path &Model::directory() const { return directory_; }
...
bool Model::GeneratePointData(const NormalRenderer &renderer,
  ...
  {
	  //cv::circle(img_show, center_vetor[index], 2, cv::Scalar(0, 0, 255), -1);
	  cv::circle(points_img, center_vetor[index], 3, cv::Scalar(255), -1);
  }
  bitwise_not(points_img, points_img);
  string samplePointsPath = "C:\\my_dataset\\18\\sampleContours\\";
  cv::imwrite(samplePointsPath +std::to_string(sample_n) + ".png", points_img);
  sample_n++;
#endif
  /*cv::namedWindow("img_show", 0);
  cv::imshow("img_show", points_img);
  cv::waitKey(0);*/
...
bool Model::GenerateValidContours(
  ...
  //cv::imshow("contours_img", silhouette_image);
...
void Model::GenerateGeodesicPoses(
  ...
  for (const auto &geodesic_point : geodesic_points) {
	...
	/*pose = pose.inverse();
	std::ofstream optimizedposefile_1("F:\\Data_pose\\00QEL_Assembly_Datasets\\part_02\\viewpoints.txt", std::ios::app);
			optimizedposefile_1 << pose(0, 0) << "\t" << pose(0, 1) << "\t" << pose(0, 2) << "\t" <<
				pose(1, 0) << "\t" << pose(1, 1) << "\t" << pose(1, 2) << "\t" <<
				pose(2, 0) << "\t" << pose(2, 1) << "\t" << pose(2, 2) << "\t" <<
				pose(0, 3) << "\t" << pose(1, 3) << "\t" << pose(2, 3) << "\t" << "\n";

	std::ofstream optimizedposefile_2("F:\\Data_pose\\00QEL_Assembly_Datasets\\part_02\\xyz.txt", std::ios::app);
	optimizedposefile_2 << pose(0, 3) << "\t" << pose(1, 3) << "\t" << pose(2, 3) << "\t" << "\n";*/
  }
}
