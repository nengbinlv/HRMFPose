...
LoaderCamera::LoaderCamera(const std::string &name,
                           const std::experimental::filesystem::path &load_directory,
...
bool LoaderCamera::UpdateImage() {
  ...
  std::experimental::filesystem::path path{load_directory_ /
  ...
  cv::remap(image_, image_, distortion_map_, cv::Mat(), cv::INTER_NEAREST, cv::BORDER_CONSTANT);
  /*矫正结束*/
...
const std::experimental::filesystem::path &LoaderCamera::load_directory() const {
...
bool LoaderCamera::LoadMetaData() {
  // Open meta data file
  std::experimental::filesystem::path path{load_directory_ / meta_filename_};
