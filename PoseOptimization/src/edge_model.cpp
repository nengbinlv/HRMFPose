...
EdgeModel::EdgeModel(const std::string &name, std::shared_ptr<Body> body_ptr,
...
bool EdgeModel::SetUp() {
  set_up_ = false;
  if (!LoadEdgeModel()) {
    if (!GenerateEdgeModel()) return false;
    if (!SaveEdgeModel()) return false;
  }
  set_up_ = true;
  return true;
}

void EdgeModel::set_name(const std::string &name) { name_ = name; }

void EdgeModel::set_body_ptr(std::shared_ptr<Body> body_ptr) {
  body_ptr_ = std::move(body_ptr);
  set_up_ = false;
}

void EdgeModel::set_directory(const std::experimental::filesystem::path &directory) {
  directory_ = directory;
  set_up_ = false;
}

void EdgeModel::set_filename(const std::string &filename) {
  filename_ = filename;
  set_up_ = false;
}

void EdgeModel::set_sphere_radius(float sphere_radius) {
  sphere_radius_ = sphere_radius;
  set_up_ = false;
}

void EdgeModel::set_n_divides(int n_divides) {
  n_divides_ = n_divides;
  set_up_ = false;
}

void EdgeModel::set_n_points(int n_points) {
  n_points_ = n_points;
  set_up_ = false;
}

void EdgeModel::set_use_random_seed(bool use_random_seed) {
  use_random_seed_ = use_random_seed;
  set_up_ = false;
}

void EdgeModel::set_image_size(int image_size) {
  image_size_ = image_size;
  set_up_ = false;
}
...
bool EdgeModel::GetClosestTemplateView(
    const Transform3fA &body2camera_pose,
    const TemplateView **closest_template_view) const {
  if (!set_up_) {
    std::cerr << "Set up EdgeModel " << name_ << " first" << std::endl;
    return false;
  }
...
const std::string &EdgeModel::name() const { return name_; }

std::shared_ptr<Body> EdgeModel::body_ptr() const { return body_ptr_; }

const std::experimental::filesystem::path &EdgeModel::directory() const { return directory_; }

const std::string &EdgeModel::filename() const { return filename_; }

float EdgeModel::sphere_radius() const { return sphere_radius_; }

int EdgeModel::n_divides() const { return n_divides_; }

int EdgeModel::n_points() const { return n_points_; }

bool EdgeModel::use_random_seed() const { return use_random_seed_; }

int EdgeModel::image_size() const { return image_size_; }

bool EdgeModel::set_up() const { return set_up_; }

//生成bin文件
bool EdgeModel::GenerateEdgeModel() {
  ...
  std::cout << "Start generating EdgeModel " << name_ << std::endl;
...
#pragma omp for
  ...
  std::cout << "Finish generating EdgeModel " << name_ << std::endl;
  return true;
}

bool EdgeModel::LoadEdgeModel() {
  ...
  if (!data_ifs.is_open() || data_ifs.fail()) {
    data_ifs.close();
    std::cout << "Could not open EdgeModel file " << data_path << std::endl;
    return false;
  }

  // Check if EdgeModel file has correct parameters
  int version_id_file;
  ...
  if (version_id_file != kVersionID || sphere_radius_file != sphere_radius_ ||
    ...
    std::cout << "EdgeModel file " << data_path
              << " was generated using different EdgeModel parameters" << std::endl;
    return false;
  }

  // Check if EdgeModel file has correct geometry data
...
bool EdgeModel::SaveEdgeModel() const {
  std::experimental::filesystem::path data_path{directory_ / filename_};
...
//bool EdgeModel::GeneratePointData(const teture_render &renderer,
...
bool EdgeModel::GeneratePointData(const NormalRenderer &renderer,
	const Transform3fA &camera2body_pose,
	std::vector<PointData> *data_points) const {
	// Compute silhouette
	std::vector<cv::Mat> normal_image_channels(4);
	...
	for (auto contour : contours)
	{
		...
		{		
			...
			index_coontour_num.push_back(index_contour);		
		}	
		index_contour++;
	}
	//去重-稍微快一些
	...
	//======进行筛选=====
#if 1
	vector<int> index_coontour_num_candidate;
	...
	while (true)
	{
		...
		if (keep)
		{
			...
			index_coontour_num_candidate.push_back(index_coontour_num[i]);
		}
		...
		if (i == (int)vector_points_all.size()) {
			...
			if (first_select) {
				if (num_ok) {
					...
					index_coontour_num_candidate.clear();
					i = 0;
					distance += 1.0f;
...
#endif
	vector<int> index_coontour_num_center;
	...
	for (int i =0;i< Candidates.size();i = i + step)
	{
		...
		index_coontour_num_center.push_back(index_coontour_num_candidate[i]);
	}
#if 1
	//得到的轮廓点数与 给定不符
	if (center_vetor.size() >= n_points_)
	{
		...
		if (n != 0)
		{
			...
			while (n != 0)
			{
				...
				index_coontour_num_center.erase(index_coontour_num_center.begin() + s);
				s = s + index_step;
				n--;
			}
		}
...
#endif
	//cout << index_coontour_num_center.size() << endl;
#if 0
	cv::Mat img_show = renderer.normal_image();
	cv::Mat points_img(silhouette_image.size(), CV_8U, cv::Scalar(0));

	for (int index = 0;index < center_vetor.size();index++)
	{
		//cv::circle(img_show, center_vetor[index],3,cv::Scalar(0,0,255),-1);
		cv::circle(points_img, center_vetor[index], 3, cv::Scalar(255), -1);
	}
	bitwise_not(points_img, points_img);
	string samplePointsPath = "C:\\my_dataset\\18\\sampleEdges\\";
	cv::imwrite(samplePointsPath + std::to_string(sample_edge_n) + ".png", points_img);
	sample_edge_n++;
	/*cv::namedWindow("show_every_point", 0);
	cv::imshow("show_every_point", points_img);
	cv::waitKey(0);*/
#endif

	/*vector<cv::Mat> channels;
	split(img_show, channels);
	...
	*/
	...
	for (auto data_point{ begin(*data_points) }; data_point != end(*data_points);) {
		...
		//cout << data_point->index_contour << endl;

		data_point->index_contour = index_coontour_num_center[kk - 1];
...
void EdgeModel::calcImageGradientDirection(cv::Mat& dst, const cv::Mat& src, const std::vector<cv::Point>& pts) const
{
...
std::vector<double> EdgeModel::calcImageGradientDirection(const cv::Mat& src, const std::vector<cv::Point>& pts) const
{
...
bool EdgeModel::extractEdgeDescriptor(cv::Mat& desc, const cv::Mat& im, cv::Point pt,
...
bool EdgeModel::GenerateValidContours(
  ...
  //===16U转8U
  /*normalize(depth_iamge_temp, depth_iamge_temp, 0, 255, cv::NORM_MINMAX);
  convertScaleAbs(depth_iamge_temp, depth_iamge_temp);*/
  depth_iamge_temp = depth_iamge_temp / 255;
  depth_iamge_temp.convertTo(depth_iamge_temp, CV_8U);

  cv::namedWindow("normal",0);
  cv::namedWindow("depth", 0);
  cv::imshow("normal", normal_image_temp);
  cv::imshow("depth", depth_iamge_temp);
  cv::waitKey(1);

  //string path_normal = "C:\\my_dataset\\18\\normal\\";
  //string path_depth = "C:\\my_dataset\\18\\depth\\";
 // cv::imwrite(path_normal + std::to_string(normal_n) + ".png", normal_image_temp);
  //cv::imwrite(path_depth + std::to_string(depth_n) + ".png", depth_iamge_temp);
  ...
  //cv::waitKey(0);
	...
	cv::Mat depth_canny;
    cv::Canny(depth_iamge_temp, depth_canny, low_thre, height_thre);
    //cv::imwrite("depth_canny.jpg", depth_canny);
    cv::Mat normal_depth_canny;
    normal_depth_canny = normal_canny + depth_canny;
#endif
	...
	//laplace 测试
	cv::Mat laplace_Mat_depth;
	//cv::GaussianBlur(normal_image_temp, normal_image_temp,cv::Size(3,3),5,0);
	cv::Laplacian(depth_iamge_temp, laplace_Mat_depth, CV_8U, 3,1,0);
	cv::convertScaleAbs(laplace_Mat_depth, laplace_Mat_depth);

	cv::threshold(laplace_Mat_depth, laplace_Mat_depth, 20, 255, cv::THRESH_BINARY);//THRESH_OTSU THRESH_BINARY
	//cv::imwrite("laplace_Mat_depth.jpg", laplace_Mat_depth);

	cv::Mat laplace_Mat_normal;
	//cv::GaussianBlur(normal_image_temp, normal_image_temp,cv::Size(3,3),5,0);
	
	cv::Laplacian(normal_image_temp, laplace_Mat_normal, CV_8U, 3, 1, 0);
	cv::convertScaleAbs(laplace_Mat_normal, laplace_Mat_normal);
	
	cv::cvtColor(laplace_Mat_normal, laplace_Mat_normal, CV_BGR2GRAY);
	cv::threshold(laplace_Mat_normal, laplace_Mat_normal, 40, 255, cv::THRESH_BINARY);
	//cv::imwrite("laplace_Mat_normal.jpg", laplace_Mat_normal);
	...
	cv::Mat depth_dilate;
	cv::dilate(laplace_Mat_depth, depth_dilate, struct_element);

	//cv::dilate(depth_canny, depth_dilate, struct_element);
	//cv::dilate(depth_dilate, depth_dilate, struct_element);
	//cv::dilate(depth_dilate, depth_dilate, struct_element);
	//cv::imwrite("depth_dilate.jpg", depth_dilate);

	//cv::ximgproc::thinning(depth_dilate, depth_dilate, cv::ximgproc::THINNING_ZHANGSUEN); //

	//cv::imwrite("depth_dilate_thinning.jpg", depth_dilate);
	//图像做减法（normal-depth_dilate）
	cv::Mat normal_sub_depthDilate;
	//normal_sub_depthDilate = laplace_Mat_normal - depth_dilate;
	normal_sub_depthDilate = laplace_Mat_normal - depth_dilate;
	
	cv::Mat laplace_depth_normal;
	//cv::imwrite("normal_sub_depthDilate.jpg", normal_sub_depthDilate);
	//laplace_depth_normal = laplace_Mat_depth + normal_sub_depthDilate;
	laplace_depth_normal = laplace_Mat_depth + normal_sub_depthDilate;

	//cv::ximgproc::thinning(laplace_depth_normal, laplace_depth_normal, cv::ximgproc::THINNING_ZHANGSUEN);
	//cv::dilate(laplace_depth_normal, laplace_depth_normal, struct_element);
	//cv::ximgproc::thinning(laplace_depth_normal, laplace_depth_normal, cv::ximgproc::THINNING_ZHANGSUEN);
	
	//cv::threshold(laplace_depth_normal, laplace_depth_normal, 40, 255,cv::THRESH_BINARY);  //40

	//image_edge = normal_image_temp;

	//cv::imwrite("laplace_depth_normal.jpg", laplace_depth_normal);
	//cv::waitKey(0);

	/*cv::namedWindow("normal_depth_img",0);
	cv::imshow("normal_depth_img", normal_depth_canny);
	cv::Mat iii = normal_depth_canny.clone();
	bitwise_not(iii, iii);
	cv::imwrite("normal_depth_canny.jpg", iii);
	cv::waitKey(0);*/
#endif
#if 0
	...
	cv::imshow("edge_image_ed", edge_image_ed_show);
	cv::waitKey(1);*/

	/*cv::imwrite("normal_depth_canny.jpg", normal_depth_canny);
	cv::namedWindow("normal_depth_canny",0);
	cv::imshow("normal_depth_canny", normal_depth_canny);
	cv::waitKey(0);*/
  // Compute contours
	//silhouette_image normal_depth_canny laplace_depth_normal
	
	std::vector<std::vector<cv::Point2i>> contours_edge;
	cv::findContours(laplace_depth_normal, contours_edge, cv::RetrievalModes::RETR_LIST,
		cv::ContourApproximationModes::CHAIN_APPROX_NONE);
  ...
  std::vector<std::vector<cv::Point2i>> contours_silhouette;
  cv::findContours(silhouette_image, contours_silhouette, cv::RetrievalModes::RETR_LIST,
	  cv::ContourApproximationModes::CHAIN_APPROX_NONE);
  contours_silhouette.erase(std::remove_if(begin(contours_silhouette), end(contours_silhouette),
	  [](const std::vector<cv::Point2i> &contour) {
	  return contour.size() < kMinContourLength;
  }),
	  end(contours_silhouette));


  for (auto contour: contours_silhouette)
  {
	  contours->push_back(contour);
  }
  for (auto contour : contours_edge)
  {
	  contours->push_back(contour);
  }

  //cv::Mat contours_img(silhouette_image.size(), CV_8U, cv::Scalar(0));
  //cv::Mat contours_img_c(silhouette_image.size(), CV_8U, cv::Scalar(0));
  //for (int i = 0; i < contours->size(); i++)
  //{
	 // cv::drawContours(contours_img, *contours, i, cv::Scalar(255), 1);	 
  //}
  //for (int i = 0; i < contours_silhouette.size(); i++)
  //{
	 // cv::drawContours(contours_img_c, contours_silhouette, i, cv::Scalar(255), 1);
  //}
  //bitwise_not(contours_img, contours_img);
  //bitwise_not(contours_img_c, contours_img_c);

  //cv::namedWindow("contours_img", 0);
  //cv::imshow("contours_img", contours_img);
  //cv::namedWindow("contours_img-c", 0);
  //cv::imshow("contours_img-c", contours_img_c);
  //cv::waitKey(0);

  //string path_edge = "C:\\my_dataset\\18\\edge\\";
  //string path_contour = "C:\\my_dataset\\18\\contour\\";
  //cv::imwrite(path_edge + std::to_string(normal_n) + ".png", contours_img);
  //cv::imwrite(path_contour + std::to_string(normal_n) + ".png", contours_img_c);
  //normal_n++;
  //depth_n++;
...
cv::Point2i EdgeModel::SampleContourPointCoordinate(
...
bool EdgeModel::CalculateContourSegment(
...
Eigen::Vector2f EdgeModel::ApproximateNormalVector(
...
void EdgeModel::CalculateLineDistances(
    const cv::Mat &silhouette_image,
    const std::vector<std::vector<cv::Point2i>> &contours,
...
void EdgeModel::FindClosestContourPoint(
...
void EdgeModel::GenerateGeodesicPoses(
...
void EdgeModel::GenerateGeodesicPoints(
...
void EdgeModel::SubdivideTriangle(
...
void EdgeModel::checkLineExtremes(cv::Vec4f& extremes, cv::Size imageSize)
{
