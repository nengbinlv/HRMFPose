...
void Camera::StartSavingImages(const std::experimental::filesystem::path &save_directory,
                               int save_index,
...
const cv::Mat &Camera::image_edge() const { return image_edge_; }

const cv::Mat &Camera::image_mask() const { return image_mask_; }

const cv::Mat &Camera::image_mask_dilate() const { return image_mask_dilate_; }
...
void Camera::SaveMetaDataIfDesired() const {
  if (save_images_) {
    std::experimental::filesystem::path path{save_directory_ / (name_ + "_meta.txt")};
...
void Camera::SaveImageIfDesired() {
  if (save_images_) {
    std::experimental::filesystem::path path{save_directory_ / (name_ + "_image_" +
