...
#ifndef OBJECT_TRACKING_INCLUDE_SRT3D_LINE_MODALITY_H_
#define OBJECT_TRACKING_INCLUDE_SRT3D_LINE_MODALITY_H_
...
namespace srt3d {
	...
	public:
		...
		LineModality(const std::string &name, std::shared_ptr<Body> body_ptr,
		...
		std::shared_ptr<LineModel> LineModel_ptr() const;
	...
	public:
		...
		void CalculateBasicLineData(const LineModel::PointData &data_point,
		...
		bool LineModality::CalculateEdgeSearchLineDistribution(float center_u, float center_v,
		...
		void VisualizePointsOcclusionMask(const std::string &title,
			int save_index) const;
		void VisualizeLines(const std::string &title, int save_index);
		...
		std::shared_ptr<LineModel> LineModel_ptr_ = nullptr;
		...
		int function_length_ = 1;                  //8 
		int distribution_length_ = 30;             //12

		std::vector<float> distribution_length_vector_{ 40.0f, 25.0f, 18.0f, 12.0f };


		std::vector<int> scales_ = {1, 1, 1, 1};      //{5, 2, 2, 1}
		int n_newton_iterations_ = 3;             //3
		float min_continuous_distance_ = 6.0f;       //6.0f
		
		float min_probility_ = 0.1;
		float variance_{};
		std::vector<float> standard_deviations_{ 15.0f, 5.0f, 3.5f, 1.5f };   //15.0f, 5.0f, 3.5f, 1.5f
		//正太分布的均值和方差
		...
		std::vector<float> sigma_global_{6.0f, 6.0f,6.0f, 6.0f};          //1.2f   2.2f
	...
	public:
			...
			//设计两种权重的系数
			float weight_response = 0.8f;
			float weight_orientation = 0.2f;
			...
			//越大 考虑的距离越远
			float tukey_norm_constant_ = 100.0f;

			std::vector<float> tukey_norm_constant_vector_ = { 80.0f, 50.0f, 30.0f, 20.0f };
