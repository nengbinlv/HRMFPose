...
class Camera {
 public:
  ...
  void StartSavingImages(const std::experimental::filesystem::path &save_directory,
                         int save_index = 0,
  ...
  const cv::Mat &image_edge() const;

  const cv::Mat &Camera::image_mask() const;

  const cv::Mat &Camera::image_mask_dilate() const;
