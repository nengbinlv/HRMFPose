...
#ifndef OBJECT_TRACKING_INCLUDE_SRT3D_EdgeModel_H_
#define OBJECT_TRACKING_INCLUDE_SRT3D_EdgeModel_H_
...
// Class that stores a sparse viewpoint EdgeModel of a body that consists of views
// with multiple contour points. It includes all functionality to generate,
// save, and load the EdgeModel
class EdgeModel {
 private:
  // Some constants
  static constexpr int kVersionID = 2;
  static constexpr int kContourNormalApproxRadius = 3; //3
  static constexpr int kMinContourLength = 20;         //15  50
  static constexpr int kMinEdgeLength = 20;         //15
 ...
 public:
  ...
  EdgeModel(const std::string &name, std::shared_ptr<Body> body_ptr,
        const std::experimental::filesystem::path &directory, const std::string &filename,
        float sphere_radius = 0.5, int n_divides = 4, int n_points = 200,//200
 ...
 private:
  // Helper methods for EdgeModel set up
  bool GenerateEdgeModel();
  bool LoadEdgeModel();
  bool SaveEdgeModel() const;
...
#endif  // OBJECT_TRACKING_INCLUDE_SRT3D_EdgeModel_H_
